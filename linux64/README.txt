...
3. Who?
-------
The author of SPTree is Antonio Frangioni from the Operations Research Group at 
the Dipartimento di Informatica of the University of Pisa [1].
...
- a build environment for source distribution install (gcc etc..)
...
4.2 Linux
---------
Untar the binary dist package pySPTree-0.1.1.linux-x86_64.tar.gz with

tar xfvz pySPTree-0.1.1.linux-x86_64.tar.gz

It will install into /usr/local/lib/python2.7/dist-packages/.

4.3 Source Distribution
-----------------------
Grab the pySPTree-0.1.1_src_dist.zip file, extract it and run
...
5. Usage
--------
Hey! Wait a minute! Please note that pySPTree's version is 0.2.0. So this is not productive! 
I tested all the relevant methods and all of my tests went fine, but if you run into an error that occurs 
in the underlying C++ library Python might crash, because at the moment I don't know yet how to deal with 
C++ exceptions and SWIG. Feel free to contact me, if you know more on this!
...
C arrays in Python? Yes - don't worry. There are helper methods in pySPTree, that'll do this for you.
...
Please check out the sample script pySPTree_test.py for more information.
...
This included:
- LoadDMX() accepts in pySPTree a string value (original: c iostream). The original LoadDMX method is omitted.
...
[1] http://www.di.unipi.it/optimize/Software/MCF.html#SPTree
