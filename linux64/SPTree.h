...
/*--------------------------------------------------------------------------*/
   ...
   inline cIndex_Set ArcPredecessors( void );
   //Johannes Sommer
...
/*
//Johannes Sommer
inline void SPTree::SetDest( int NewDst )
{
	if ( NewDst == -1)
	{
		MCFClass::cIndex NewDst = Inf<MCFClass::cIndex>();
		if( Dest != NewDst + USENAME0 ) {
			#if( LABEL_SETTING )
			Dest = NewDst + USENAME0;
			#endif
...
//Johannes Sommer moved here from SPTree.cpp
